...
macOS向けのdotfiles。パッケージ、各ツールの設定、エージェントの共通規約・スキル・MCPを管理する。
...
- エージェントへの指示：このプロジェクトはルートの `AGENTS.md`、全プロジェクト共通は `codex_global/AGENTS.md`。`claude_global/` は2026年9月以降deprecated（Claude Proのサブスク解約したため）
