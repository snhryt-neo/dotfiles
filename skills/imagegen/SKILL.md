---
name: imagegen
description: "Codex の組み込み image_gen ツール（gpt-image-2）で画像を生成し、カレントディレクトリに保存する。ユーザーが /imagegen にプロンプトを指定して実行したとき、または画像生成・画像作成を明示的に依頼したときに使う。API 呼び出しには非対応。"
---
...
`--size` 未指定時は、用途や構図から以下の候補を参考に選ぶ。納品先の必須寸法など、推測すると用途を満たせなくなる不明点がある場合だけ確認する。
...
ユーザーが指定した人物・文字・ロゴなどは、その指定に沿って生成する。テイストに複数の候補があることや「おしゃれに」などの表現だけを理由に停止せず、用途と上記のデフォルトで補完する。必須の文言や参照対象が不明で成果物の正否が変わる場合だけ、生成前にまとめて確認する。
